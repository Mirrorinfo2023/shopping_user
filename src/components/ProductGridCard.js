...
const ProductGridCard = ({ product, userId = "12345", onUpdate }) => {
  ...
  // Get the product ID with proper validation
  const productId = product?._id || product?.id;

  // Check initial wishlist and cart status on component mount
  useEffect(() => {
    if (productId) {
      checkWishlistStatus();
      checkCartStatus();
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          const isProductInCart = data.data.cartItems.some(item => 
  ...
  const calculateDiscount = () => {
    if (!product?.price || !product?.finalPrice || product.price <= product.finalPrice) return 0;
    return Math.round(((product.price - product.finalPrice) / product.price) * 100);
  };
      ...
      if (isInCart) {
        ...
        const cartItem = {
          ...
          variant: product.variants && product.variants.length > 0 
            ? `${product.variants[0]?.variantName}: ${product.variants[0]?.value}`
            : "Default",
          quantity: 1,
          price: product.price,
  ...
  // If no product or product ID, don't render anything
    ...
    <Link href={`/products/${productId}`} className="block">
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-100 group relative">
        ...
        <button
          ...
          disabled={loadingWishlist || !productId}
        ...
        <div className="p-3">
          ...
          {/* Product Name */}
          <h3 className="font-semibold text-sm text-gray-800 mb-2 line-clamp-2 h-10 leading-tight">
            {productName}
          </h3>
          <h5 className="font-light text-gray-600 truncate w-full">
  {productDescription || "No description available"}
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <button
              ...
              disabled={loadingCart || !productId}
